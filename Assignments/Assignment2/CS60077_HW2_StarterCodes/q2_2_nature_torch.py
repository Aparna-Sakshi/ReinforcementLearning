...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        strides = [4,2,1]
        padding = lambda kernel,stride : ((stride - 1) * img_height - stride + kernel) // 2
        paddings = [padding(kernel,stride) for kernel,stride in zip(ks,strides)]


        self.q_network = nn.Sequential(
            nn.Conv2d(channels,32,kernel_size=ks[0] ,stride=strides[0], padding=paddings[0]),
            nn.ReLU(),
            nn.Conv2d(32,64,kernel_size=ks[1] ,stride=strides[1], padding=paddings[1]),
            nn.ReLU(),
            nn.Conv2d(64,64,kernel_size=ks[2] ,stride=strides[2], padding=paddings[2]),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(4096, 512),
            nn.ReLU(),
    ...
    def get_q_values(self, state, network):
        ...
        state_transpose = torch.transpose(state, 1, 3)   
        if network == 'q_network':
            out = self.q_network(state_transpose)
        else:
            out = self.target_network(state_transpose)        
        ##############################################################
        ######################## END YOUR CODE #######################
        return out
